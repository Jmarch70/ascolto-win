...
## 2026-08-17 — System-audio silence watchdog, after a call recorded 68 minutes of "Me" and nothing else

User's 2026-08-13 EliseAI virtual onsite call recorded cleanly end-to-end (no errors, no crash — journal shows a normal start/pause/resume/stop/transcribe cycle) but `system.wav` came out ~3.9MB against a ~725MB `mic.wav` for the same 68-minute session. Per `audio_capture.py`'s existing loopback design (only writes packets while audio is actually playing), that means the loopback tap heard almost nothing all call. Root cause is believed to be a device-role mismatch: `system_device_index: None` resolves to Windows' *default* playback device at record-start, but call apps (Zoom/Teams/Meet) often route audio to the separate *default communications* device instead, which can point at a different endpoint even when both are nominally the same headphones. Nothing in the app could tell, because the failure throws no exception — the stream stayed open and healthy the whole time, just silent.

Rather than trying to nail the exact Windows-side cause, added a structural catch: a **silence watchdog** (`App._silence_watchdog` in `app.py`, backed by a new `CallRecorder.system_bytes_written()` counter in `audio_capture.py`). While a recording is active (not paused), it polls the system-audio byte count every 15s; if it goes flat for 60+ seconds it fires a tray notification telling the user to check that their call app's output device matches Ascolto's. It warns once per distinct silent stretch — resets when real audio resumes, doesn't count paused time against the threshold, and won't spam repeatedly for an unresolved, ongoing stretch. Verified the state-machine logic (fires once per stretch, ignores pause time, re-arms after genuine recovery, no false positives) against a fake recorder/icon harness with the interval/threshold scaled down to run in ~2 seconds, rather than trusting it unread — all six checks passed.

This doesn't fix the underlying Windows audio-routing behavior (that's on the user to check next time: Sound settings' "Default" vs "Default Communication" device, and the call app's own speaker setting, should all match) — it just means a repeat of this failure gets caught within ~60–75 seconds of it starting instead of being discovered after the call ends with an unusable transcript.
